...
def analyze_semantics(node):
    try:
        ...
        if node_type == 'program':
            ...
            if len(node) == 4:
                # Extract the actual function name from the identifier non-terminal
                fun_name = node[1][1]
            ...
            if lookup_symbol(fun_name):
                ...
                declare_symbol(
                    fun_name, {'type': 'function', 'params': params})
            ...
            if len(param_names) != len(set(param_names)) or len(param_types) != len(set(param_types)):
                raise Exception(
                    "Error: Function has multiple parameters with the same name and datatype")
            ...
            # Extract the actual variable name from the identifier non-terminal
            var_name = node[2][1]
            ...
            if len(node) == 4:
                ...
                analyze_semantics(
                    ('assignment', var_type, var_name, init_value))
            ...
            if node[1][0] == 'general_type' or node[1][0] == 'list_type' or node[1][0] == 'array_type':
                ...
                if not lookup_symbol(var_name):
                    ...
                    raise Exception(f"Error: Variable {
            ...
            if len(node) == 3 and node[2][0] == 'function_call' or len(node) == 4 and node[3][0] == 'function_call':
                ...
                if isinstance(assigned_value, str) and var_type != 'string':
                    raise Exception(f"Error: Type mismatch. Expected {
                    var_type}, got string")
        ...
        #     pop_scope()
            ...
            if expr_type is None:
                raise ValueError(
                    f"Invalid expression in print statement: {node[1]}")
            ...
            if fun_info is None:
                ...
                if num_args != num_params:
                    raise Exception(
                        f"Error: Number of arguments in function call ({
                        num_args}) does not match the number of "
            ...
            if in_function is False:
                raise Exception(
                    f" Values may only be returned from a function")
        ...
        # If an exception occurs during semantic analysis, print the error
